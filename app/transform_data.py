...
def data_interview(nombre: str, data: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    """
    ...
    """
    # Limpia el nombre de la finca y obtiene su latitud, longitud y tamaño.
    data_finca = setle_clean(nombre)

    # Filtra las áreas y perímetros que se encuentran dentro de la finca.
    prueba = filter_area_perimetro(
        data, data_finca["latitud_c"], data_finca["longitud_c"], data_finca["hectares"]
    )
...
def add_dormida_column(
    df: pd.DataFrame, cluster_val: Any, start_time: int, end_time: int
) -> pd.DataFrame:
    """
...
def cosa(numero_horas: float) -> str:
    """
